...
function getQuiz() {
  ...
  const month = today[1];
  // today quiz
  const today_sheets = XLSX.readFile(
    "./rsc/today/" + year + month + "_revoicequizlist.xlsx",
  ...
  // ad quiz
  const ad_sheets = XLSX.readFile(
    "./rsc/ad/" + year + month + "_ad_revoicequizlist.xlsx",
    { dateNF: "yyyy-mm-dd", cellDates: true, encoding: "utf8" }
  );
...
app.use("/revoice", async function(req, res, next) {
  ...
  // 현재 시간이 정각이고
  if (
    Math.floor(moment.duration(nowTime.diff(checkTimeStart)).asHours()) == 0
  ) {
    // 10분이 되기 전이면
    if (
      Math.floor(moment.duration(nowTime.diff(checkTimeStart)).asMinutes()) < 10
    ) {
